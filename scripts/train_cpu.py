"""
Training Script - CPU Version

For systems with limited GPU memory.
Uses CPU which is slower but more reliable.
"""
...
class Trainer:
    ...
    def train(self):
        """Main training loop."""
        print("\n" + "="*70)
        print("TRAINING ON TEST SAMPLES (CPU)")
        print("="*70)
...
def main():
    ...
    print("\n" + "="*70)
    print("MULTIMODAL FER - CPU TRAINING (ULTRA-LIGHTWEIGHT)")
    print("="*70)
    print("⚠️  Using CPU - Training will be slower but more reliable")
    print("    Expected time: ~30-60 minutes for 50 epochs")
    print("="*70)
    ...
    config = {
        ...
        "save_dir": "checkpoints/test_samples_cpu",
    }
    ...
    print("\n[1/4] Creating ultra-lightweight model...")
    print("  Optimized for CPU training")
    
    # Ultra-lightweight configs
    audio_config = AudioBranchConfig(
        feature_dim=256,  # Very small
    ...
    visual_config = VisualBranchConfig(
        ...
        feature_dim=256,  # Very small
        temporal_depth=2,  # Minimal
    )
    
    fusion_config = LFM2FusionConfig(
        use_pretrained=False,  # Custom LFM2
        num_layers=2,  # Minimal
        hidden_dim=512,  # Small
        audio_dim=256,
        visual_dim=256,
        output_dim=512,  # Match classifier input
    )
    ...
    print("⏱️  This will take ~30-60 minutes on CPU. Please be patient...")
    trainer.train()
